...
echo "$newid" > './server_list/"$fileid".conf' &&		## Adds the Server IP and Username to a file with a random number(id#) for a filename
		gpg -c './server_list/"$fileid".conf' &&	## Encrypts the new file and deletes the old file
...
echo "$nickname" >> 'serverlist' &&				## Adds the new alias to the serverlist
		gpg -c 'serverlist' && rm 'serverlist' &&	## Re-Encrypts the serverlist and deletes the old list
echo "$nickname"= "$fileid" >> 'identity.conf' &&		## Adds the Alias and the Credentials File ID into the Identity List
	...
	rm 'identity.conf'					## Removes the old List
